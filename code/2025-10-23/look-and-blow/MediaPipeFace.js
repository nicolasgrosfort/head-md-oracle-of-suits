...
function _flipResultsForSelfie(results) {
  ...
  if (Array.isArray(out.multiFaceLandmarks)) {
    out.multiFaceLandmarks = out.multiFaceLandmarks.map((fl) =>
      _flipLandmarkArray(fl)
    );
  }
...
async function createFaceModel(opts = {}) {
  ...
  try {
    ...
    console.warn(
      "MediaPipeFace: failed to create FaceLandmarker via tasks-vision:",
      err
    );
    ...
    if (window.FaceLandmarker) {
      try {
        window.face = new window.FaceLandmarker({
          locateFile: (f) =>
            `https://cdn.jsdelivr.net/npm/@mediapipe/face_landmark/${f}`,
        });
        faceReady = true;
        console.log(
          "MediaPipeFace: created instance using window.FaceLandmarker fallback."
        );
        return window.face;
      } catch (e) {
        console.warn(
          "MediaPipeFace: fallback FaceLandmarker instantiation failed:",
          e
        );
      }
    }
    if (window.FaceMesh) {
      try {
        ...
        faceReady = true;
        console.log(
          "MediaPipeFace: created instance using window.FaceMesh fallback."
        );
        return window.face;
      } catch (e) {
        console.warn(
          "MediaPipeFace: fallback FaceMesh instantiation failed:",
          e
        );
      }
    }
...
async function setupFace(opts = {}) {
  try {
    ...
    if (typeof inst.onResults === "function") {
      ...
      faceSendFrame = null;
      console.log(
        "setupFace: onResults not found; will use detect/detectForVideo patterns."
      );
    }
  } catch (err) {
...
function setupVideo(selfieMode = true, width = 640, height = 480) {
  ...
  if (!window.Camera) {
    console.warn(
      "MediaPipe Camera util not found. Ensure camera_utils is loaded."
    );
    return;
  }
    ...
    onFrame: async () => {
      ...
      try {
        if (typeof window.face.detectForVideo === "function") {
          const res = await window.face.detectForVideo(
            videoElement.elt,
            performance.now()
          );
        ...
        console.warn(
          "No known face model method found (send/detect/detectForVideo).",
          Object.keys(window.face)
        );
      } catch (err) {
...
function onFaceResults(results) {
  if (window.selfieModeActive) {
    try {
      ...
      console.warn(
        "Failed to flip results for selfie mode, using original results:",
        e
      );
    }
  }
...
function getFaceLandmarks() {
  ...
  return (
    detections.multiFaceLandmarks ||
    detections.faceLandmarks ||
...
function getFaceFeature(featureKeyOrIndices, faceIndex = 0, toPixels = true) {
  ...
  if (typeof featureKeyOrIndices === "string") {
    ...
    Array.isArray(featureKeyOrIndices) &&
    typeof featureKeyOrIndices[0] === "number"
  ) {
  ...
  for (const idx of indices) {
    ...
    if (!lm) continue;
    out.push(
      toPixels ? markToPixel(lm, videoElement.width, videoElement.height) : lm
    );
  }
...
function getFeatureRings(featureKey, faceIndex = 0, toPixels = true) {
  ...
  for (const e of connectors) {
    ...
    else if (e && typeof e.start === "number" && typeof e.end === "number")
      add(e.start, e.end);
  }
  ...
  const ringsPts = ringsIdx
    .map((ring) =>
      ring.map((idx) => {
        const lm = landmarks[idx];
        return toPixels
          ? markToPixel(lm, videoElement.width, videoElement.height)
          : lm;
      })
    )
    .filter((r) => r && r.length);
...
function getBlendshapeList(faceIndex = 0) {
  ...
  return bs.categories
    .map((c) => ({
      name: c.categoryName || c.displayName || "unknown",
      score: c.score || 0,
    }))
...
function listFaceModelConstants() {
  const FaceClass = window.face && window.face.constructor;
  if (!FaceClass) return null;
  return Object.keys(FaceClass).filter(
    (k) => typeof FaceClass[k] !== "undefined"
  );
}
