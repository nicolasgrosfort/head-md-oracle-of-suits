...
function drawDebugInfo() {
  ...
  if (handDetected) {
    ...
    text(
      `Hand raw: X=${indexTip.x.toFixed(3)}, Y=${indexTip.y.toFixed(
        3
      )}, Z=${indexTip.z.toFixed(4)}`,
      10,
      20
    );
    text(
      `Hand mapped: X=${handX.toFixed(0)}, Y=${handY.toFixed(
        0
      )}, Z=${handZ.toFixed(0)}`,
      10,
      40
    ...
    text(
      `Smoothed: X=${smoothHandX.toFixed(0)}, Y=${smoothHandY.toFixed(
        0
      )}, Z=${smoothHandZ.toFixed(0)}`,
      10,
      60
    );
    text(
